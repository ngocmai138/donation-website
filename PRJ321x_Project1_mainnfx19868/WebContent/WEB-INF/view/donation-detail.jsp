...
<f:form modelAttribute="pagination" action="${pageContext.request.contextPath }/admin/detailDonation" method="get" id="userDonationForm">
...
<table>
	...
	<c:url var="changeStatus" value="changeStatusUD">
		...
		<tr>
			...
			<td>${u.money }</td>
			<td>${u.created }</td>
			<td>${u.donation.description }</td>
			<td>${u.status == 0 ? "Chờ xác nhận":"Đã xác nhận"}</td>
			<td><input type="button"  onclick="window.location.href='${changeStatus}'" value="${u.status == 0 ? 'Xác nhận':'Hủy xác nhận'}"/></td>
		</tr>
	</c:forEach>
</table>
<c:forEach var="i" begin="1" end="${totalPages }">
	<a href="${pageContext.request.contextPath }/admin/detailDonation?pageSize=${pageSize}&pageNumber=${i}&donationId=${donation.id}">${i }</a>
</c:forEach>
</f:form>
<br>
<input type="button" value="Quay lại" onclick="window.location.href='listDonation'"/>
<br><br>
<script>
